...
Outputs are written under the review bundle's phase14 directory so the
release package is reusable from a clean checkout once private inputs are
remapped into private_inputs/.
"""
...
SRC_FALLBACK = REPO / "release" / "data" / "slow_stars_expanded_candidates_vgrf50.csv"
XDIR = BUNDLE / "phase14" / "expanded_spectroscopic_crossmatch"
...
def main() -> int:
    ...
    src = pd.read_csv(SRC if SRC.exists() else SRC_FALLBACK)
