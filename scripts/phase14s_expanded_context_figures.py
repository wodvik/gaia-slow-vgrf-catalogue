...
and regenerates the review-bundle context figures:
...
def draw_edge(df: pd.DataFrame) -> None:
    panels = [
        (r"Current positions ($R_{\rm GC}, Z_{\rm GC}$)", "R_current_kpc", "z_current_kpc", "vgrf_default_exact", 0, 25),
...
def write_summary(df: pd.DataFrame) -> None:
    payload = {
        ...
        "figures": [bundle_rel(EDGE_PDF), bundle_rel(FACE_PDF)],
        "cache": bundle_rel(CACHE),
    }
