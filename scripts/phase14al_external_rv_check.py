...
def write_table(summaries: list[dict]) -> None:
    lines = [
        ...
        r"\tablecaption{External radial-velocity consistency audit for the subset of"
        ...
        r" coverage is footprint-selected and is used only as a robustness check."
        r"\label{tab:external_rv}}",
        r"\tablehead{\colhead{survey} & \colhead{$N_{\rm ABC}$} &"
        r" \colhead{$N_{\rm AB}$} & \colhead{median $\Delta RV$} &"
        r" \colhead{robust scatter} & \colhead{$|\Delta RV|>5$}}",
        r"\startdata",
    ]
    for s in summaries:
