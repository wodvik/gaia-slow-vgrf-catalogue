"""Phase 14AD -- stronger smooth-DF null models for the slow-tail excess.

Referee response (deep-review Issue 3). The manuscript's smooth-tail
expectation fits a single trivariate-Gaussian velocity ellipsoid to the
matched-control library (|v_GC| in 25--260 km/s) and predicts
N(<25)=76.5, implying a 25.5x excess over the observed Tier A+B+C count
...
excess under three smooth nulls fit to the SAME control velocity vectors:
...
evaluated by Monte Carlo sampling of the fitted model, and the excess
factor is 1952 / N_pred. Bootstrap resampling of the control library gives
a 16th--84th percentile interval on each excess factor, and an isotropic
velocity-error jitter checks measurement-error sensitivity.

The robust conclusion is that the slow tail is strongly overabundant
relative to ANY smooth velocity field (excess >> 1 in every null); the
precise factor is null-dependent, so the single-ellipsoid 25.5x is a
heuristic baseline and the BIC mixture gives a conservative lower bound.
...
def _latex_table(summ: dict) -> None:
    ...
    lines = [
        ...
        r" ($N=1{,}952$ below $25\kms$) under three smooth velocity-distribution nulls"
        ...
        r" ranges over the control library. The excess is large under every null, but"
        r" its magnitude is null-dependent: the single ellipsoid is a heuristic baseline"
        r" and the BIC mixture, which best fits the control velocity field, gives the"
        r" conservative floor.\label{tab:null_models}}",
