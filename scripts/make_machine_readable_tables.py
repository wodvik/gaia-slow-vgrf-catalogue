...
def _write_mrt(path: Path, title: str, table: Table) -> None:
    ...
    for row in table:
        ...
        for name, _, _ in COLUMNS:
            ...
            parts.append(text.rjust(widths[name]) if name not in {"dist_source", "rv_quality", "rvs_quality_ok", "tier"} else text.ljust(widths[name]))
...
def _write_sample_tex(path: Path, table: Table) -> None:
    ...
    lines.extend([
        r"\enddata",
        r"\tablecomments{Only eight rows are shown for form and content. The electronic table contains all 517 Tier~A+B sources.}",
        r"\end{deluxetable*}",
    ])
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    _write_sample_tex(TABLE_OUT / "tab_headline_catalogue_sample.tex", tier_ab)
    return 0
