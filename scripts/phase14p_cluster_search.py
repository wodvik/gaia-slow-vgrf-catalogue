...
CATALOGUE = BUNDLE / "catalogues" / "catalogue_expanded_master.fits"
ORBITS = BUNDLE / "catalogues" / "catalogue_expanded_orbits_tierABC.fits"
OUT_DIR = BUNDLE / "phase14"
...
def write_markdown(path: Path, summaries: list[dict], group_rows: list[dict]) -> None:
    ...
    tier_ab = by_sample["tierAB_primary"]
    tier_abc = by_sample["tierABC_statistical"]
    lines.extend([
        "",
        ...
        f"with small relative velocities. The closest primary Tier A+B pair is "
        f"{tier_ab['min_pair_sep_pc']:.1f} pc apart; Tier A+B contains "
        f"{tier_ab['pair_counts']['pairs_sep_lt_25pc']} pair below 25 pc, and "
        f"Tier A+B+C contains {tier_abc['pair_counts']['pairs_sep_lt_25pc']} pairs below 25 pc.",
...
def main() -> None:
    ...
    samples = {
        "tierAB_primary": np.isin(tier, ["A", "B"]),
    ...
    search_config = {
        "inputs": {
            "catalogue": str(CATALOGUE.relative_to(BUNDLE)),
            "orbits": str(ORBITS.relative_to(BUNDLE)),
        },
        "searches": SEARCHES,
        "summaries": summaries,
